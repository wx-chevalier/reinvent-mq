...
import (
	...
	"wx.pongo/pongo/fsm"
	"wx.pongo/pongo/metadata"
	"wx.pongo/pongo/structs"
	"wx.pongo/pongo/util"
	"wx.pongo/log"
	"wx.pongo/protocol"
)
...
func init() {
	...
	e := os.Getenv("pongoDEBUG")
	if strings.Contains(e, "broker=1") {
		brokerVerboseLogs = true
	}
}

// Broker represents a broker in a pongo cluster, like a broker in a Kafka cluster.
type Broker struct {
	sync.RWMutex
	...
	// The raft instance is used among pongo brokers within the DC to protect operations that require strong consistency.
	raft          *raft.Raft
...
func (b *Broker) becomeFollower(replica *Replica, cmd *protocol.PartitionState) protocol.Error {
	...
	conn, err := NewDialer(fmt.Sprintf("pongo-replicator-%d", b.config.ID)).Dial("tcp", broker.BrokerAddr)
	if err != nil {
		return protocol.ErrUnknown.WithErr(err)
	}
