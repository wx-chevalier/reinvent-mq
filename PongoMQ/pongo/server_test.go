...
func TestProduceConsume(t *testing.T) {
	...
	s1, dir1 := pongo.NewTestServer(t, func(cfg *config.Config) {
		cfg.Bootstrap = true
	}, nil)
	...
	pongo.WaitForLeader(t, s1)

	s2, dir2 := pongo.NewTestServer(t, func(cfg *config.Config) {
		cfg.Bootstrap = false
	}, nil)
	...
	s3, dir3 := pongo.NewTestServer(t, func(cfg *config.Config) {
		cfg.Bootstrap = false
	}, nil)
	...
	pongo.TestJoin(t, s1, s2, s3)
	controller, others := pongo.WaitForLeader(t, s1, s2, s3)
	...
	controller, others = pongo.WaitForLeader(t, others...)
...
func BenchmarkServer(b *testing.B) {
	...
	defer cancel()
	srv, dir := pongo.NewTestServer(b, func(cfg *config.Config) {
		cfg.Bootstrap = true
...
func createTopic(t ti.T, s1 *pongo.Server, other ...*pongo.Server) error {
	d := &pongo.Dialer{
